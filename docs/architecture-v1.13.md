# 接力教育智慧云平台 - 系统架构与目录结构规范（V1.13）

## 1. 文档信息

- 架构版本：`v1.13`
- 对齐需求：`docs/exam-platform-requirements-spec-v1.7.md`
- UI 规范：`docs/design-system-v1.4.md`
- 技术栈：`docs/tech-stack.md`
- 更新时间：`2026-03-29 16:00:00`

本文件用于统一约束项目的系统分层、目录放置、页面与组件归档、左侧菜单命名、API 约定以及数据库结构。后续所有前端、后端、数据库相关开发，默认以本文件为准。

本版本重点补充：
- 明确评卷老师与指导老师为独立角色，支持同一账号同时拥有双角色
- 新增师生关联管理菜单规范（导入管理下新增关联管理子菜单）
- 补充教师-学生关联关系的数据库核心关系说明
- 细化各层职责与数据流转路径
...
├─────────────────────────────────────────────────────────────────────────────┤
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐     │
│  │   学生训练端  │  │   学生考试端  │  │   教师端     │  │  管理后台    │     │
│  │  Vue3 SPA    │  │  Vue3 SPA    │  │  Vue3 SPA    │  │  Vue3 SPA    │     │
│  │              │  │              │  │              │  │              │     │
│  │ • 学情看板   │  │ • 极简登录   │  │ • 学情分析   │  │ • 考务监控   │     │
│  │ • 模拟训练   │  │ • 答题矩阵   │  │ • 智能组卷   │  │ • 账号管理   │     │
│  │ • 错题本     │  │ • 实时保存   │  │ • 资源管理   │  │ • 评卷管理   │     │
...
│  └──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘     │
│                                                                              │
│  技术栈: Vue3 + TypeScript + Vite + Pinia + Vue Router 4 + Tailwind CSS      │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
│   ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐             │
│   │   负载均衡       │  │   SSL 终止      │  │  静态资源缓存    │             │
│   │  (Round Robin)  │  │  (HTTPS/443)    │  │  (JS/CSS/IMG)   │             │
│   └─────────────────┘  └─────────────────┘  └─────────────────┘             │
│                                                                              │
│   ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐             │
│   │   限流熔断       │  │   跨域处理      │  │  请求日志记录    │             │
│   │  (Rate Limit)   │  │   (CORS)        │  │  (Access Log)   │             │
│   └─────────────────┘  └─────────────────┘  └─────────────────┘             │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
...
│   │                        核心领域服务                                  │   │
│   │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐  │   │
│   │  │ 账号服务  │ │ 题库服务  │ │ 组卷服务  │ │ 考试服务  │ │ 评卷服务  │  │   │
│   │  │ Account  │ │ Question │ │  Paper   │ │  Exam    │ │ Review   │  │   │
│   │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘  │   │
│   │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐  │   │
│   │  │ 成绩服务  │ │ 资源服务  │ │ 激活服务  │ │ 导入服务  │ │ 审计服务  │  │   │
│   │  │  Score   │ │ Resource │ │Activation│ │  Import  │ │  Audit   │  │   │
│   │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘  │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                        基础支撑服务                                  │   │
│   │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐  │   │
│   │  │ 认证中心  │ │ 权限控制  │ │ 日志审计  │ │ 文件存储  │ │ 消息推送  │  │   │
│   │  │   Auth   │ │   RBAC   │ │   Log    │ │  MinIO   │ │ WebSocket│  │   │
│   │  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘  │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
...
│   │   ┌─────────────┐    ┌─────────────┐    ┌─────────────┐            │   │
│   │   │  交卷队列    │    │  算分队列    │    │  通知队列    │            │   │
│   │   │Submit Queue │───▶│ Score Queue │───▶│Notify Queue │            │   │
│   │   │  (削峰)     │    │  (异步)     │    │  (广播)     │            │   │
│   │   └─────────────┘    └─────────────┘    └─────────────┘            │   │
│   │                                                                     │   │
│   │   场景: 开考 1000-3000 QPS 时，交卷请求先入队，消费者异步处理算分    │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
...
│                                                                              │
│   ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐             │
│   │    MySQL 8.x    │  │   Redis 7.x     │  │    MinIO        │             │
│   │    主数据库      │  │   缓存/会话     │  │   对象存储       │             │
│   │                 │  │                 │  │                 │             │
│   │ • 用户账号      │  │ • Session共享   │  │ • 主观题附件    │             │
│   │ • 题库试题      │  │ • 答题卡缓存    │  │ • 视频解析      │             │
│   │ • 考试记录      │  │ • 字典缓存      │  │ • 评分表扫描件  │             │
│   │ • 成绩数据      │  │ • 分布式锁      │  │ • 资源文件      │             │
│   │ • 操作日志      │  │ • 限流计数      │  │                 │             │
...
| 层级 | 技术组件 | 核心职责 |
|------|----------|----------|
| **客户端层** | Vue 3 + TypeScript + Vite + Pinia + Vue Router 4 | 页面展示、交互、路由守卫、假数据演示、表单校验和可视化反馈 |
| **API 网关层** | Nginx | 负载均衡、SSL 终止、静态资源缓存、限流熔断、跨域处理 |
| **应用服务层** | Java 21 + Spring Boot 3.x + Spring Security + RabbitMQ | 鉴权、业务编排、数据持久化、日志审计、成绩计算、异步削峰 |
| **数据存储层** | MySQL 8.x + Redis 7.x + MinIO | 核心业务数据存储、缓存加速、文件分片存储 |

### 2.3 关键业务数据流转

```
┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│  学生考试    │────▶│  实时保存    │────▶│   交卷提交   │────▶│  RabbitMQ   │
│  答题界面    │     │  (Redis缓存) │     │             │     │  消息队列    │
└─────────────┘     └─────────────┘     └─────────────┘     └──────┬──────┘
                                                                    │
                                                                    ▼
┌─────────────┐     ┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│  成绩发布    │◀────│  主观题合分  │◀────│  客观题算分  │◀────│  异步消费者  │
│  (定时触发)  │     │  (平均分)   │     │  (即时计算)  │     │             │
└─────────────┘     └─────────────┘     └─────────────┘     └─────────────┘
```
...
- 管理后台、教师端、评卷端的表格、表单、树形、日期选择优先基于 `Element Plus`
- 学生训练端与考试端页面优先基于 `Tailwind CSS` 做定制化实现
...
| 导入管理 | 名单导入 / 名单补全 / 关联管理 | 报名导入、教师关联、关联查看调整 |
...
│   └── 关联管理/              # teacher-student relation
│       ├── 关联列表/          # view relations
│       ├── 批量关联/          # batch relation import
│       └── 关联调整/          # modify relations
...
- `sys_user` 与 `sys_role` 通过 `sys_user_role` 多对多关联（支持同一教师账号同时拥有指导老师、评卷老师双角色）。
- 教师账号与学生训练账号通过 `teacher_student_relation` 一对多关联（一个老师可带多个学生，一个学生只能关联一个指导老师）。
...
- 先做布局、路由和假数据，再接真实后端。
- 前端实现必须遵循 `docs/design-system-v1.2.md`。
