...
### 6.1 图标技术选型
* **必须使用 SVG 图标**，禁止使用 Emoji 或图片格式的图标。
* SVG 图标优势：矢量无损缩放、支持 CSS 样式控制、文件体积小、可动态修改颜色。

### 6.2 图标使用规范
* **图标库**：推荐使用 `@element-plus/icons-vue` 或自定义 SVG 图标组件。
* **引入方式**：
  ```vue
  <!-- 正确：使用 SVG 图标组件 -->
  <el-icon><User /></el-icon>
  <svg-icon name="check-circle" class="w-5 h-5" />
...
* **尺寸规范**：
  ...
  * 标准图标：`20px` (`w-5 h-5`)
  * 大图标：`24px` (`w-6 h-6`)
  * 统计卡片图标：`28px` (`w-7 h-7`，置于 56px 圆形背景内)

### 6.3 图标颜色控制
* 图标颜色应继承当前文本颜色或使用语义色类名（如 `text-blue-500`、`text-emerald-500`）。
* 通过 `currentColor` 实现与文本颜色同步变化。

---

## 7. 特殊场景组件设计 (Special Components)

* **统计卡片 (Stat Card)**：
  * **图标风格**：采用**面性图标**设计（白色 SVG 线条图标 + 渐变圆形背景）
  * **图标尺寸**：56px × 56px 圆形背景，图标 28px × 28px
  * **渐变配色**：
    * 蓝色系：`linear-gradient(135deg, #3B82F6, #6366F1)` —— 用于用户/账号相关
    * 绿色系：`linear-gradient(135deg, #10B981, #059669)` —— 用于成功/登录相关
    * 橙色系：`linear-gradient(135deg, #F59E0B, #D97706)` —— 用于警告/场次相关
    * 红色系：`linear-gradient(135deg, #EF4444, #DC2626)` —— 用于错误/异常相关
  * **图标规范**：使用 SVG 路径绘制，stroke="white" stroke-width="2"，不使用 emoji
  * **常用图标**：
    * 用户/账号：双人头轮廓（users icon）
    * 登录/进入：登录箭头（login icon）
    * 检查/完成：对勾圆圈（check-circle icon）
    * 日历/场次：日历轮廓（calendar icon）
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.4 | 2026-03-29 | 新增第6节"图标规范"，明确规定必须使用 SVG 图标，禁止使用 Emoji |
| v1.3 | 2026-03-28 | 新增统计卡片图标规范，细化面性图标设计标准 |
| v1.2 | - | 统计卡片渐变配色规范 |
| v1.1 | - | 考试专用视觉规则 |
| v1.0 | - | 初始版本 |
